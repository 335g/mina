...
use crossterm::event::{KeyCode, KeyEvent, KeyModifiers as Modifiers};
...
impl Keymaps {
    ...
    fn movement_bindings(extend: bool) -> Vec<(Vec<KeyEvent>, Command)> {
        ...
        // v12 再構築のチューニング決定: 素の `G` は診断 view（TUI オーバーレイ）に
        // 譲り、キーマップからは外す。末尾へは `g e` で到達できる。Select では
        // daemon 側が Extend 扱いする。
        out.push((
...
mod tests {
    ...
    fn modes_have_separate_keymaps() {
        ...
        assert!(matches!(
            km.resolve(Mode::Insert, &mut pending, KeyCode::Esc.into()),
    ...
    fn uppercase_keys_normalize_shift() {
        // crossterm は大文字バイトを Char('G')+SHIFT で報告する → 正規化して一致
        let km = Keymaps::new();
        let mut pending = Vec::new();
        // v12: 素の G は診断 view（TUI オーバーレイ）のためキーマップにはない
