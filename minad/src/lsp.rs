...
const SEMANTIC_RETRY_WAIT: Duration = Duration::from_millis(0); // iteration #6: 500ms → 0ms（ADR-0054）。意味的リクエストは解析完了までブロックしてから返るため待ちは不要。2 回連続同一の確認と SEMANTIC_RETRIES の予算は残す
