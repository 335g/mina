...
1. **外部からの変更を黙って消さない**（主眼） —— 編集の最中に別プロセスが同じファイルを書き換える状況を注入した実験で、**209 run 中 1 件だけ**変更を失い（それは全文を書き戻す arm）、minas の 2 経路は **0/51**。その失敗モードが実在すること自体は、決定論的なテストで毎回検証しています。
2. **大きいファイルで費用が増えない**（支える主張） —— 4,800 行/ファイルのタスクで入力トークンは素朴な全文 read 比 **−79%**、コスト **−61%**（中央値）。素朴なツールは規模に比例して増え、minas はほぼ横ばい。交差点は 300〜600 行/ファイル。
...
エージェントにとって重要な帰結が 2 つあります: **編集がデーモンの状態に対して検証される**こと、そして**外部変更が検知されリロードされる**ことです。どちらも「静かに壊さない」ための土台で、下の[主張 1](#主張-1-静かに壊さない)で計測します。
...
（arm の定義は次の節にあります。ここでは `native` = opencode 標準のファイルツール、`naive` = 全文 read + 無検証の全体置換、
`minas` = 検証付き `apply`、`positional` = 位置指定 `edit`）
...
- **交差点は 300〜600 行/ファイル**。それより小さいファイルでは minas のほうが高い（150 行では +3%）。
  つまり「いつも得する」ツールではありません。**得するのは、1 ファイルが大きいとき**です。
- 最大規模では **入力 −70〜−83%、コスト −51〜−61%**（同一 idx の paired 比）。
...
（比率は同一 idx の paired 比の中央値。上の表の「中央値どうしの比」とは少し違います）
...
逆に、**1 ファイルが 300 行未満の作業では minas は払い損**です（費用の図の左端）。
