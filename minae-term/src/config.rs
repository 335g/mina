...
//! `minae config` サブコマンド（本モジュールの [`ConfigCmd`]）はこのファイルの
...
/// 設定ファイルのパス: `$XDG_CONFIG_HOME/minae/config.toml`（なければ `~/.config/minae/config.toml`）。
...
fn show(effective: bool) -> io::Result<()> {
    ...
    match std::fs::read_to_string(&path) {
        ...
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            ...
            eprintln!("hint: `minae config init` で雛形を作成できます");
        }
        Err(e) => return Err(e),
    }
    Ok(())
...
fn effective_toml(config: &Config) -> String {
    let mut out = String::from("# 実効設定 (minae config show --effective)\n");
...
fn get(key: &str) -> io::Result<()> {
    ...
    let text = std::fs::read_to_string(&path).map_err(|e| {
        if e.kind() == io::ErrorKind::NotFound {
            invalid("設定ファイルがありません（minae config init で作成）")
        } else {
            e
        }
    })?;
...
pub const TEMPLATE: &str = "# minae 設定ファイル\n\n\
