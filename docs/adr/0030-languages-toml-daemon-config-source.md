...
- **LSP セッション spawn 時**（`ensure` の 1 箇所）: 毎回再読込。ファイルは数 KB で
  spawn 自体が数秒かかるためコストは無視できる。編集した languages.toml は**次に
  新しいセッションが spawn される時点で反映**され、稼働中セッションはネゴシエーション
  済みのまま（再 initialize しない）。
