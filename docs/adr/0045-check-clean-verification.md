...
「解析完了を LSP 応答から確実に検知する手段は無く」の部分は**半分だけ**
正しかった。索引の完走は `$/progress`（work-done progress）の購読で検知できる
（ADR-0051。`window.workDoneProgress` を advertise しないと rust-analyzer は
進捗を送らない、が実測で判明）。

ただし**本書の判断は維持する**。索引完走後（warm・idle）の pull でも実在する
エラーを取りこぼすことを実測した（L0 計測 2026-09-12、小クレート）:

- config.rs のフィールドを消し main.rs の参照が壊れている状態 → `check main.rs` は
  空 + exit 0（`check config.rs` は検出できる）
...
片付けた（計測の記録: `docs/loop/log.md` の iteration #3）。
