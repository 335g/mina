...
> **注意（2026-09-16 追記）**: 本ドキュメントの入力トークン／コストの数値は、**改名前の
> `minae session` CLI・旧 runner・旧計器**で測ったもので、現在の計器では再現できない。
> **公開している費用・安全性の主張は [l2/README.md](l2/README.md)（t11 の掃引、
> モデル・runner・n・日付つき）**を参照。ここは当時の検証の記録として残している。
...
| `docs/loop/` | **L0（LLM なし・決定論的）**のフロー費用計測と、改善ループの回し方・全反復の記録。まず `docs/loop/README.md` |
| **本ファイル** | 総括・スキル反映マップ |
