//! mina-debug: `mina session edit` の試行用対話 CLI（開発用・使い捨て）。
//!
...
//!
//! 使い方: `cargo run -p mina-debug`（daemon 未起動なら自動起動する）
...
fn run() -> std::io::Result<()> {
    cliclack::intro("mina-debug: DocumentEdit 試行ツール")?;
...
/// daemon が起動していなければ `mina daemon serve` を起動して待つ。
/// `mina` バイナリは mina-debug の隣（target/debug 配下）→ PATH の順で探す。
fn ensure_daemon(path: &Path) -> std::io::Result<()> {
    ...
    let exe = find_mina().ok_or_else(|| {
        std::io::Error::new(
            ErrorKind::NotFound,
            "`mina` バイナリが見つかりません（`cargo build -p mina-term` で生成、または PATH に追加）",
        )
    })?;
...
fn find_mina() -> Option<PathBuf> {
    let sibling = std::env::current_exe().ok()?.parent()?.join("mina");
    if sibling.exists() {
        return Some(sibling);
    }
    std::env::var_os("PATH").and_then(|path| {
        std::env::split_paths(&path)
            .map(|dir| dir.join("mina"))
            .find(|p| p.exists())
    })
}
